...
public class Application implements ClientSocketListener {
	...
	private void handleCommand(String cmdLine) {
		...
		if(tokens[0].equals("quit")) {	
			...
			// check for two value, space separated may not be the best implementation

			if(tokens.length >= 3) {
				
				// be mindful of the case where the key or value have a terminating char \0
				...
				if(client != null && client.isRunning()){
					...
					for(int i = 2; i < tokens.length; i++) {
						...
						if (i != tokens.length -1 ) {
							value.append(" "); // may want to handle whitespace a bit differently, rn all whitespace is going to turn into spaces
						}
					}	
