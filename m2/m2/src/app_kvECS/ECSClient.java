...
public class ECSClient implements IECSClient {
	...
	public ECSClient( boolean startFailureDetection) {
		...
		try {
			...
			ProcessBuilder pd = new ProcessBuilder("apache-zookeeper-3.8.0-bin/bin/zkServer.sh", "stop");
			Process p = pd.start();

			ProcessBuilder pd2 = new ProcessBuilder("apache-zookeeper-3.8.0-bin/bin/zkServer.sh", "start");
			...
			try {
				...
				if (zk.exists(zk_crashedNodePath,true) == null) {
					IECSNode dummy = new ECSNode("127.0.0.1", 0);
					//TODO: may have to change data
					zk.create(zk_crashedNodePath, Serializer.nodeToBytes(dummy), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
				}
			} catch (KeeperException e) {
				logger.error("Zookeeper error setting up ActiveNode: " + e);
			}

			if(startFailureDetection) new CrashDetection(this).start();
		...
		try{
			if(zk.exists(zk_crashedNodePath, true) != null) {
				byte[] data = zk.getData(zk_crashedNodePath, new Watcher(){
					@Override
					public void process(WatchedEvent we) {handleServerCrash();}
				}, null);
				/*
Callback function of type Watcher. The ZooKeeper ensemble will notify through the
Watcher callback when the data of the specified znode changes. This is one-time notification.
...
//					addNode("FIFO", 123);
	...
	public void parseECSConfig(){
		try {
			...
			while (scanner.hasNextLine()) {
				...
				availablePorts.push(new ECSNode(configLine[1], Integer.parseInt(configLine[2])));
			}
	...
	private void runSSH(String node_host, int node_port, String cacheStrategy, int cacheSize){
		...
		String script = "ssh -n " + node_host + " nohup java -jar " + path+"/m2-server.jar " +
        ...
        try {
			...
			if(serverProcessInput == null){
				serverProcessInput = new BufferedReader(new
						InputStreamReader(p.getInputStream()));

				serverProcessError = new BufferedReader(new
	...
	private void transferNodeDataOnAdd(String curr) {
		...
		if (servers_online == 1) return;

		//first clear all data that is in the node to ensure against failures or anything
	...
	public IECSNode addNode(String cacheStrategy, int cacheSize) {
		...
		try {
			...
			if (node == null) throw new Exception("Unable to add new node: Ran out of ports");
			//run server
			...
			//TODO: Check the zookeeper error; currently has no effect to adding/removing node
			try {
...
//		updateAllNodeMetadata(); //update metadata on the server to reflect changed metadata here
        return new_nodes;
	}
	...
	public boolean awaitNodes(int count, int timeout) throws Exception {
		// TODO: the strat is actually to do nothing here, becuase actually awaiting is going to add a painful amount of code
        ...
        while (System.currentTimeMillis() - start_time < timeout)
            if (zk.getChildren(zk_activeNodePath, true).size() == servers_online + count) return true;
        return false;
	}
        ...
        while (i.hasNext()) {
			...
			try {
				if (nodes.containsKey(k)) {
					//need to find the next largest to move all the data from teh current node to the next node
	...
	@Override
	public Map<String, IECSNode> getNodes() {return nodes;}

	@Override
	public IECSNode getNodeByKey(String Key) {return nodes.get(Key);}
	...
	private void sendMessage(String k, KVAdminMessage kvAdminMessage){
		...
		try {
			System.out.print("Sending Message: " + kvAdminMessage.message + " to port " + nodes.get(k).getNodePort() + " ");
	...
	private void sendMoveMessage(String srcServerKey, String dstServerKey, String nextUpperBound,
		...
		sendMessage(srcServerKey, new KVAdminMessage(
				"Move data from "+nodes.get(srcServerKey).getNodePort() + " to " + nodes.get(dstServerKey).getNodePort(),
