...
public class InteractionTest extends TestCase {
	...
	public void setUp() {
		kvClient = new KVStore("localhost", 100);
	...
	public void testPut() {
		...
		try {
			response = kvClient.put(key, value, true, false);
		} catch (Exception e) {
			ex = e;
		}
	...
	public void testPutDisconnected() {
		...
		try {
			kvClient.put(key, value, true, false);
		} catch (Exception e) {
			ex = e;
		}
		try {
			kvClient.put(key, "null",true, false);
		} catch (Exception e) {
			ex2 = e;
		}
		try {
	...
	public void testUpdate() {
		...
		try {
			kvClient.put(key, initialValue, true, false);
			initialRetrievedValue = kvClient.get(key).getValue();
			response = kvClient.put(key, updatedValue, true, false);
	...
	public void testDelete() {
		...
		try {
			kvClient.put(key, value, true, false);
			response = kvClient.put(key, "null", true, false);
