...
public class KVServer extends Thread implements IKVServer {
	 ...
	 *                   "LRU",
	 *                   and "LFU".
	 * @param connectECS True = enables ecs connection, false = disables ecs
	 *                   connection
	 ...
	 * @param forcePut true = only from moving data, key guaranteed to exist, skips
	 *                 lock
	 *                 false = everywhere else
	 * @throws Exception
	 */
	@Override
	public void putKV(String key, String value, boolean forcePut, boolean replicate, boolean fromClient)
			throws Exception {
		...
		if (this.writeLock == true && !forcePut) {
			...
			if (replicate) {
				// TODO: Look to deprecate replicate and forceput with replacement of
				// responsible server

				// if(responsibleServer.equals(this.hashedName)) { //used for testing
				// should always be force putting replicas on successors
				if (!successor_1.equals(hashedName))
					replicaPut(key, value, true, successor_1);
				if (!successor_2.equals(hashedName))
					replicaPut(key, value, true, successor_2);
			}
		} else {
			throw new Exception("1 Not in Server Bounds");
		}
	...
	/**
	 * @param dstServerHashName the hash name of the server in which keys should be
	 *                          moved to
	 ...
	 * @param replicate         true = replicate moved data to other servers; do so
	 *                          for actual run
	 *                          false = do not replicate; used for unit testing
	 *                          moveData
	 * @throws Exception
	 *                   Assumptions:
	 *                   - Metadata is correct; dstServerHashName can be found in
	 *                   metadata
	 *                   - On Add: Metadata updated before moveData
	 *                   - On Delete: Metadata updated after moveData
	 *
	 *                   - Server bounds is circumvented through (force) boolean var
	 *                   which forces adds despite out of boundary
	 *
	 *                   m3 Additions:
	 *                   -> boolean field to update replicas
	 *                   -> if yes, also need to put data and remove data from
	 *                   replicas
	 */
	public void moveData(String dstServerHashName, String hashUpperBound, boolean delete, boolean replicate)
			throws Exception {
		if (dstServerHashName.equals(hashedName))
			return;
		this.writeLock();
